...
Use Python 3.10+ and install the analysis/test dependencies from:

```powershell
python -m pip install -r requirements.txt
```

The dependency file declares `numpy`, `matplotlib`, `scipy`, and `pytest`.
...
- All compared parallel configurations have lower mean `global_best` than SERIAL.
- Paired statistics show positive SERIAL-vs-parallel mean differences for all 6 comparisons; 4 remain `p<0.05` after Holm-Bonferroni correction across all pairwise paired t-tests, while Welch sensitivity analysis gives `p<0.05` for all 6 SERIAL comparisons.
- Version A best tours are saved under `results/version_a_best_tours/` and verified with `scripts/verify_version_a_tours.py`.
...
The fixed-total-population experiment is documented by:
...
Run the equal-budget experiment:
