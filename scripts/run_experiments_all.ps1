param(
    ...
    [string]$ResultsFile = "results/all_experiment_results.csv"
)
...
foreach ($path in @($InputPath, $SerialSource, $DeaSource, $HdeaSource)) {
    if (-not (Test-Path -LiteralPath $path)) {
...
if (Test-Path -LiteralPath $ResultsPath) {
    ...
    Write-Host "backed up existing all-experiment results to $backup"
}
