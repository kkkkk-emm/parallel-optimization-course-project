...
static void append_csv(double globalBest, double elapsedSec, double migrationCommSec, double finalCollectiveCommSec)
{
    ...
    if (needHeader) {
        fprintf(fp, "algorithm,nproc,maxGen,migration_interval,base_seed,global_best,elapsed_sec,migration_comm_sec,final_collective_comm_sec,mpi_comm_sec,computation_sec,comm_ratio\n");
    }

    fprintf(fp, "DEA,%d,%ld,%ld,%lu,%.0f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",
